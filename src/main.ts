...
import { OnvifHelper } from './onvif-helper';
import type {
...
class ReoLinkCamAdapter extends Adapter {
    ...
    private onvifHelper: OnvifHelper | null = null;
    private visitorClearTimer: ioBroker.Timeout | undefined = undefined;
     ...
     */
    private async startDoorbellOnvif(): Promise<void> {
        ...
        this.onvifHelper = new OnvifHelper(
            {
                host,
                port,
        ...
        try {
            ...
            this.log.error(
                `Could not start ONVIF listener for doorbell (${host}:${port}): ${error instanceof Error ? error.message : error}`,
            );
            this.log.error(
                'Check that ONVIF is enabled on the camera and the ONVIF port is correct (Reolink default: 8000).',
            );
     ...
     */
    private async handleVisitorEvent(active: boolean): Promise<void> {
     ...
     */
    async getAudioFileList(): Promise<void> {
        if (!this.reolinkApiClient) {
            return;
        }
        try {
     ...
     * Doorbell cameras only.
     */
    async getAutoReply(): Promise<void> {
        if (!this.reolinkApiClient) {
            return;
        }
        try {
     ...
     * Doorbell cameras only.
     */
    async setAutoReply(): Promise<void> {
        ...
        const command: ReolinkCommandSetAutoReply[] = [
            {
                cmd: 'SetAutoReply',
                param: {
                    AutoReply: {
     ...
     */
    async quickReplyPlay(id: number): Promise<void> {
        const command: ReolinkCommandQuickReplyPlay[] = [
            {
                cmd: 'QuickReplyPlay',
                action: 0,
                param: {
        ...
        if (state) {
            if (!state.ack) {
                ...
                if (propName == 'ir') {
                    ...
                    await this.quickReplyPlay(parseInt(state.val as string, 10));
                } else if (
                    propName === 'autoReplyEnabled' ||
     ...
     */
    private async createDoorbellStates(): Promise<void> {
     ...
     */
    private async removeDoorbellStates(): Promise<void> {
    ...
    private readonly doorbellObjectIds = [
