...
[[cameras]]
name = "${config.name}"
username = "${escapeTOML(config.username)}"
password = "${escapeTOML(config.password)}"
uid = "${config.uid}"
...
[cameras.mqtt]
  ...
  ${config.mqttUser ? `username = "${escapeTOML(config.mqttUser)}"` : ''}
  ${config.mqttPassword ? `password = "${escapeTOML(config.mqttPassword)}"` : ''}
  enable_motion = true
  enable_battery = true
